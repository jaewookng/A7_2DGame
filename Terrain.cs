...
public class Terrain
{
    ...
    public Terrain(Texture2D tilemap, int tileSize, int tileScale)
    {
        ...
        // first layer
        AddLayer(0, -1, 0, -1); //2 platforms, ground on left
        //second layer
        AddLayer(-1, 0, 0, -1); //1long platform
    ...
    public void Draw(SpriteBatch spriteBatch)
    {
        for (int j = 0; j < _layers.Count; j++)
        {
            for (int i = 0; i < mapPathBottom.Length; i++)
            {
                ...
                //400 as h for now
                Vector2 position = new Vector2(i * (_tileSize * _tileScale), 536);
